...
case class TwirlHtmlLayout(
  id : Symbol,
  description : String = "",
  template: Layout.TwirlHtmlTemplate
) extends Layout[Html] {
...
case class TwirlTextLayout(
  id : Symbol,
  description : String = "",
  template: Layout.TwirlTxtTemplate
...
object Layout extends Registry[Layout[_]] {
  ...
  type TwirlHtmlTemplate = { def apply(args: Map[String,(Node,Result[_])])(implicit ctxt: Context):Html }
  type TwirlTxtTemplate = { def apply(args: Map[String,(Node,Result[_])])(implicit ctxt: Context):Txt }

  lazy val default = TwirlHtmlLayout('default, "Default Layout", scrupal.core.views.html.pages.defaultLayout)
}
