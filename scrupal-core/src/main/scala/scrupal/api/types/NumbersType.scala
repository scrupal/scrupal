...
case class NumbersType(
  ...
  override def validate(ref : Location, value : NumbersType.Numbers) : Results[NumbersType.Numbers] = {
    value.map(numbersValidation).select[Option[String]].getOrElse(None) match {
      case None ⇒ Success(ref, value)
      case Some(x : String) ⇒ StringFailure(ref, value, x)
    }
  }
  private object numbersValidation extends Poly1 {
  ...
  private def errormsg(v : Double) = Some(s"Value $v not in [$min,$max].")
}

object NumbersType {
  type Numbers = Byte :+: Short :+: Int :+: Long :+: Float :+: Double
}
