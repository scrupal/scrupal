...
trait Storable extends Serializable with Equals {
  ...
  class LongQueryable extends Queryable[Long] { def apply() : Long = primaryId }

  val qId : Queryable[Long] = new LongQueryable
  def queryables : Map[Symbol,Queryable[_]] = Map('id → new Queryable[Long] { def apply() : Long = primaryId })
}
