 ...
 */
case class Configuration(underlying: Config) extends ScrupalComponent {
  ...
  private def readValue[T](path: String, v: => T): Option[T] = {
    try {
      ...
      case NonFatal(x) => throw reportError(path, x.getMessage, Some(x))
    }
  }
   ...
   */
  def reportError(path: String, message: String, e: Option[Throwable] = None): ScrupalException = {
   ...
   * @return a configuration exception
   */
  def globalError(message: String, e: Option[Throwable] = None): ScrupalException = {
 ...
 * The underlying implementation is provided by https://github.com/typesafehub/config.
 */
object Configuration extends ScrupalComponent {
  ...
  def from(data: Map[String, Any]) = {
    ...
    def asJavaRecursively[A](data: Map[A, Any]): Map[A, Object] = {
      data.mapValues {
    ...
    val param = asJavaRecursively[String](data).asJava
    val config: Config = ConfigFactory.parseMap(param)
  ...
  private def configError(origin: ConfigOrigin, message: String, e: Option[Throwable] = None): ScrupalException = {
    toss("Configuration error " + message, e)

/*    new ScrupalException.ExceptionSource("Configuration error", message, e.orNull) {
