...
class ConfigHelper(config : Configuration) extends ScrupalComponent {
  ...
  def getInstance[C<:AnyRef](name: String)(implicit m: Manifest[C]) : Option[C] = {
    try
    {
      ...
      case x: IllegalAccessException => log.error("Cannot access class " + name + " while instantiating: ", x); None
      case x: InstantiationException =>  log.error("Cannot instantiate uninstantiable class " + name + ": ", x); None
      case x: ExceptionInInitializerError => log.error("Instance initialization of " + name + " failed: ", x); None
      case x: SecurityException => log.error("Security exception while instantiating " + name + ": ", x);  None
      case x: LinkageError => log.error("Linkage error while instantiating " + name + ": ", x); None
      case x: ClassNotFoundException =>  log.error("Cannot find class " + name + " to instantiate: ", x); None
