...
/**
 * Created by reidspencer on 10/11/14.
 */
trait ScrupalComponent {
  val identity: String = this.getClass.getCanonicalName

  val log = Logger({ if (identity == null) "scrupal.utils.NoCanonicalName" else identity })
  ...
  extends java.lang.Exception(message, cause)
{
  ...
  override def getMessage = {
    super.getMessage() + " in " + component.identity
  }
}
